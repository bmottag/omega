				...
				<div class="form-group text-left">
					...
					<select name="notification" id="notification" class="form-control">
						...
						<?php for ($i = 0; $i < count($notificationsList); $i++) { ?>
							<option value="<?php echo $notificationsList[$i]["id_notification"]; ?>" <?php if ($information && $information[0]["fk_id_notification"] == $notificationsList[$i]["id_notification"]) {
																											echo "selected";
																										}  ?>><?php echo $notificationsList[$i]["notification"]; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
				...
				<div class="form-group text-left">
					...
					<select name="emailTo" id="emailTo" class="form-control">
						...
						<?php for ($i = 0; $i < count($workersList); $i++) { ?>
							<option value="<?php echo $workersList[$i]["id_user"]; ?>" <?php if ($information && $information[0]["fk_id_user_email"] == $workersList[$i]["id_user"]) {
																							echo "selected";
			...
			<div class="col-sm-6">
				<div class="form-group text-left">
					...
					<select name="smsTo" id="smsTo" class="form-control">
						<option value=''>Select...</option>
						<?php for ($i = 0; $i < count($workersList); $i++) { ?>
							<option value="<?php echo $workersList[$i]["id_user"]; ?>" <?php if ($information && $information[0]["fk_id_user_sms"] == $workersList[$i]["id_user"]) {
																							echo "selected";
																						}  ?>><?php echo $workersList[$i]["first_name"] . ' ' . $workersList[$i]["last_name"]; ?></option>
						<?php } ?>
					</select>
